...
export default function SaleProductPage() {
    ...
    useEffect(() => {
        if (data?._id && !isLoading) {
            const isStatus =
                data?.business?.days &&
                data?.business?.openingTime &&
                data?.business?.closingTime
                    ? isBusinessOpen({
    ...
    const self = user?._id === data?.business?.creator?._id;
    ...
    return (
        <LoadingLayout loading={isLoading}>
            <div className=" w-full flex flex-col gap-4 lg:py-10 pt-6 lg:p-10  ">
                ...
                <p className=" text-sm font-medium capitalize lg:flex hidden  ">
                    Home • Product • {data?.name}
                </p>
                <div className=" w-full flex gap-6 lg:flex-row  flex-col-reverse ">
                    <div className=" w-full flex flex-col gap-4 px-4 ">
                        ...
                        <div className=" pb-4 border-b w-full flex flex-col gap-3 ">
                            <UserCard
                                item={data?.business?.creator as IUserDetail}
                                showDetail
                            />
                            {!self && (
                                <div className=" pl-10 flex flex-col gap-3 ">
                                    ...
                                    <div className=" w-full flex gap-2 ">
                                        <CustomButton
                                            onClick={() =>
                                                router.push(
                                                    `/profile/${data?.business?.creator?._id}`,
                                                )
                                            }
                                        ...
                                        <MessageBtn
                                            ...
                                            business={
                                                data?.business as IBusinessDetails
                                            }
                                            creator={
                                                data?.business
                    ...
                    <div className=" lg:w-[80%] w-full flex gap-6 ">
                        <div className=" w-full flex flex-col gap-4 lg:px-0 px-4 ">
                            ...
                            <div className=" w-full flex gap-1 items-center ">
                                <StarRating
                                    rating={Number(data?.business?.rating)}
                                />
                                <p className=" font-medium text-sm ">
                                    {/* {data?.business?.rating} Ratings •{" "}
                                    <span
                                        ...
                                        {data?.colors?.map((item) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={` w-8 h-8 rounded-full flex justify-center items-center text-gray-200 `}
                                                >
                                                    {isSelected && (
                                                        <TiTick
                                                            size={"20px"}
