                    ...
                    <div className=" w-full flex flex-col px-4 lg:px-8 " >
                        <div className=" w-full max-w-[1276px] flex flex-col gap-4 pb-5 " >
                            <div className=" flex gap-3 pb-4 border-b items-center " >
                                <button onClick={() => router.back()} className=" w-10 h-10 lg:w-12 lg:h-12 rounded-full flex  border items-center justify-center border-gray-100 text-primary " >
                                    <IoArrowBackOutline size={"22px"} />
                                </button>
                                <p className=" lg:text-2xl font-bold capitalize " >Profile</p>
                                {userData?._id === id && (
                                    <div className=" ml-auto  " >
                                        <CustomButton onClick={() => router.push(`/profile/${id}/edit`)} variant="outlinebrand" height="45px" >Edit Profile</CustomButton>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className=" w-full flex-col flex gap-6 " >
                            {data?.about && (
                                <div className=" flex flex-col " >
                                    ...
                                    <p className=" text-sm max-w-[615px] " >{data?.about}</p>
                                </div>
                            )}

                            <div className=" w-full flex gap-4 " >
                                <CustomButton onClick={() => setSelectedTab("services")} variant={selectedTab === "services" ? "primary" : "outline"} height="40px" className={` !font-medium ${selectedTab === "services" ? "!bg-[#F7F2FF] !text-brand  " : ""} `} >Services</CustomButton>
                                <CustomButton onClick={() => setSelectedTab("store")} variant={selectedTab === "store" ? "primary" : "outline"} height="40px" className={` !font-medium  ${selectedTab === "store" ? "!bg-[#F7F2FF] !text-brand  " : ""}  `}>Product</CustomButton>
                            </div>
                            <Suspense fallback={<div className=" py-4 lg:p-4 text-center ">Loading...</div>}>
                                {selectedTab === "services" && (
                                    <BusinessServices isProfile={userData?._id === id} businessId={data?.business?._id} />
                                )}
                                {selectedTab === "store" && (
                                    <BusinessProduct isProfile={userData?._id === id} businessId={data?.business?._id} />
                                )}
                            </Suspense>
                        </div>
                    </div>
