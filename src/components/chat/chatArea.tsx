...
import { messageDeleted } from "@/store/comment";
...
export default function ChatArea({
    ...
    const deletedMessage = useAtomValue(messageDeleted);
    ...
    return (
        <div
            className={` ${id ? " flex " : " lg:flex hidden "} w-full h-full flex flex-1 `}
        >
                        ...
                        >
                            <div className="h-full flex flex-1 flex-col relative">
                                ...
                                {!messages.length ? (
                                    ...
                                    <div
                                        ref={scrollRef}
                                        className="w-full flex-1 flex flex-col-reverse gap-4 p-6 overflow-auto"
                                    >
                                        {[...messages]
                                            ?.filter(
                                                (item) =>
                                                    !deletedMessage.includes(
                                                        item?._id,
                                                    ),
                                            )
                                            .sort(
...
const EmptyState = () => {
    return (
        <div className="w-full h-full flex-1 flex flex-col gap-1 justify-center items-center">
            ...
            <p className="text-xs">
                Select a chat from the left side to continue
            </p>
        </div>
    );
};
