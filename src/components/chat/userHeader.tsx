...
export default function UserHeader({ selected }: { selected: IChatList }) {
            ...
            {user?._id !== selected?.recipient?._id ? (
                <div className=" flex items-center gap-2 ">
                    ...
                    <div className=" w-fit h-fit rounded-full bg-gray-300 ">
                        <Avatar
                            src={selected?.recipient?.profilePicture}
                            name={selected?.recipient?.firstName}
                        />
                    </div>
                        ...
                        <p className=" lg:text-xl font-medium capitalize ">
                            {textLimit(
                                selected?.recipient?.firstName +
                                    " " +
                                    selected?.recipient?.lastName,
                                20,
                            )}
                        </p>
                        <p className=" text-xs text-secondary ">
                            Last seen{" "}
                            {moment(selected?.updatedAt + "").fromNow()}
                        </p>
                    </div>
                </div>
            ) : (
                <div className=" flex items-center gap-2 ">
                    ...
                    <div className=" w-10 h-10 rounded-full bg-gray-300 ">
                        <Avatar
                            src={selected?.sender?.profilePicture}
                            name={selected?.sender?.firstName}
                        />
                    </div>
                    <div className=" flex-col flex ">
                        <p className=" text-xl font-medium capitalize ">
                            {selected?.sender?.firstName +
                                " " +
            ...
            >
                <div className=" w-full flex flex-col gap-4 items-center py-4 ">
                    ...
                    <div className=" flex flex-col text-center items-center ">
                        ...
                        <p className=" text-sm leading-tight max-w-[80%] ">
                            This will permanently delete the chat. Are you sure
                    ...
                    <div className=" w-full flex flex-col mt-3 gap-2 ">
                        <CustomButton
                            isLoading={isLoading}
                            onClick={() =>
                                deleteChatMutation.mutate(selected?._id)
                            }
