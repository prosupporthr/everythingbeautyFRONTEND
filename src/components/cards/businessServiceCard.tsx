...
export default function BusinessServiceCard({
    ...
    return (
        <div
            onClick={handleClick}
            className={` cursor-pointer flex flex-col border ${option ? " w-full " : selected === item?._id ? " bg-brand text-white w-full " : "  w-full "} rounded-[10px] text-left h-full justify-center `}
        >
            <div
                className={` w-full flex gap-3 ${option ? "border-b h-[102px] px-6 " : " p-3 "} items-center gap-2  `}
            >
                <div className=" w-fit ">
                ...
                <div className=" flex-col flex ">
                    <p
                        className={` lg:w-[100px] capitalize font-semibold wrap-break-word text-sm `}
                    >
                        {textLimit(item?.name, 15)}
                    </p>
